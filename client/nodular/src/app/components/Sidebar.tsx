  ...
  return (
    <aside className="flex h-screen w-72 flex-col bg-slate-800 p-4 shadow-2xl">
          ...
          {boards.map((board) => (
            <li key={board.id}>
              <a
                ...
                className={`group flex cursor-pointer items-center justify-between rounded-md px-3 py-2 text-sm font-medium transition-colors ${
                  activeBoardId === board.id ? 'bg-slate-700/80 text-white' : 'text-slate-300 hover:bg-slate-700/50'
                }`}
              >
                {board.name}
      ...
      <div className="mt-auto space-y-2 pt-4">
        <div className="glass-pane rounded-lg p-2">
          <label className="text-xs text-slate-400">Active LLM</label>
          <div className="flex items-center justify-between">
            <select
              value={selectedLLM}
              onChange={(e) => onSelectLLM(e.target.value as LLMProvider)}
              className="w-full appearance-none border-none bg-transparent font-semibold text-white focus:outline-none"
            >
